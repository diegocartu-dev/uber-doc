...
**Cerrado (actualizado el 10/08/2026).** Las dos condiciones que faltaban se cumplieron: el gate no-muro salió en el PR #263 (banner en el dashboard + página dedicada `/medico/identidad` con salida "Volver al panel", recordatorio al médico por mail y badge para el admin) y el monitor de médico trabado, en el #378. El gate se re-prendió el 14/07 y **se volvió a apagar el mismo día**: la biometría funcionó de punta a punta en la prueba en vivo de Diego, pero la UX de espera del resultado confundía, y eso disparó el rediseño integral del registro con la biometría adentro (spec en `docs/specs/2026-07-14-rediseno-registro-medico.md`, implementada ese mismo día). Candado crítico que trajo el #263 (gate Roberto): el trigger `proteger_verificacion_medico` se extendió para que `identidad_validada`/`biometria_exenta`/`didit_*`/`refeps_validado`/`es_cuenta_test` NO sean auto-escribibles por el médico — sin eso el gate era decorativo. **Estado final: el gate está PRENDIDO desde el 20/07/2026** por orden de Diego, con las condiciones cumplidas (ver `docs/sprints/2026-08-06-cierre-terminal-registro-medico.md`).
