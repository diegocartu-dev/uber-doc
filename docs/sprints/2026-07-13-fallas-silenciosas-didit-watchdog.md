...
El gate `identidad_gate_activa` sigue APAGADO. Para re-prenderlo quedan: (a) gate no-muro (dejar entrar al dashboard y empujar a validar), (b) monitor de "médico trabado >X h". La condición "Didit completa end-to-end" ya está probada con médicos reales.
