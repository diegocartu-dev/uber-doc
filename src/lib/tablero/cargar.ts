...
export async function cargarTablero(opts: OpcionesCarga = {}): Promise<DatosTablero> {
  ...
  const datos: DatosTablero = {
    ...
    avisos: avisosRaw.filter((a) => a.medico_id && !testMed.has(String(a.medico_id))).map((a) => { const c = String(a.created_at); const f = fechaARdeISO(c); return { medicoId: String(a.medico_id), fecha: f, hora: horaARdeISO(c), min: minutoARdeISO(c), mes: mesDe(f), disparador: str(a.disparador), resultado: str(a.resultado), entrega: str(a.twilio_status) }; }),
