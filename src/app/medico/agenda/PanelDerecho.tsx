...
export default function PanelDerecho({ medicoId, precio }: { medicoId: string; precio: number }) {
  ...
  function goDia(fecha: string) { setLunesActual(getLunes(new Date(fecha + "T12:00:00"))); }
  ...
  const diasConRes = new Set<string>();
  for (const t of turnosMes) {
    if (t.estado === "disponible") diasConDisp.add(t.fecha);
    if (t.estado === "reservado") diasConRes.add(t.fecha);
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <span className="inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[11px] font-medium" style={{ background: "#E1F5EE", color: "#1D9E75" }}>● {disponibles}</span>
        <span className="inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[11px] font-medium" style={{ background: "#E6F1FB", color: "#378ADD" }}>● {reservados.length}</span>
        <span className="inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[11px] font-medium" style={{ background: "#fef2f2", color: "#A32D2D" }}>● {bloqueados}</span>
        <button onClick={goHoy} className="rounded-full bg-[#1D9E75]/10 px-2.5 py-1 text-[11px] font-medium text-[#1D9E75]">Hoy</button>
      </div>

      {/* Calendario mensual */}
      <div className="rounded-xl bg-white p-4" style={{ border: b }}>
        <div className="flex items-center justify-between">
          <button onClick={mesPrev} className="rounded px-2 py-1 text-sm text-gray-500 hover:bg-gray-100">←</button>
          <p className="text-[13px] font-medium text-gray-800">{MESES[mesVisible]} {anioVisible}</p>
          <button onClick={mesNext} className="rounded px-2 py-1 text-sm text-gray-500 hover:bg-gray-100">→</button>
        </div>
        <div className="mt-3 grid grid-cols-7 gap-0.5">
          {DIAS_LABEL.map((d) => <div key={d} className="text-center text-[11px] font-medium text-gray-500">{d}</div>)}
        </div>
        <div className="mt-1 grid grid-cols-7 gap-0.5">
          {Array.from({ length: startPad }).map((_, i) => <div key={`p${i}`} className="h-8" />)}
          {Array.from({ length: totalDias }).map((_, i) => {
            ...
            return (
              <button
                key={dia}
                onClick={() => goDia(fecha)}
                className={`relative flex h-8 items-center justify-center rounded text-[14px] cursor-pointer transition-all duration-100 ${
                  esHoy ? "bg-[#1D9E75] text-white font-medium rounded-full"
                  : "text-gray-800 hover:bg-gray-100"
                } ${enSemana && !esHoy ? "font-medium" : ""}`}
                style={enSemana && !esHoy ? { background: "#E1F5EE" } : undefined}
              >
                {dia}
                {(diasConDisp.has(fecha) || diasConRes.has(fecha)) && (
      ...
      <div className="overflow-hidden rounded-xl bg-white" style={{ border: b }}>
        <div className="flex items-center justify-between px-4 py-2.5" style={{ borderBottom: b }}>
          <button onClick={semPrev} className="rounded px-2 py-1 text-sm text-gray-500 hover:bg-gray-100">←</button>
          <p className="text-[11px] font-medium text-gray-600">
            {new Date(fechaDesde + "T12:00:00").getDate()}/{new Date(fechaDesde + "T12:00:00").getMonth() + 1} — {new Date(fechaHasta + "T12:00:00").getDate()}/{new Date(fechaHasta + "T12:00:00").getMonth() + 1}
          </p>
          <button onClick={semNext} className="rounded px-2 py-1 text-sm text-gray-500 hover:bg-gray-100">→</button>
        </div>
        ...
        {cargando ? (
          ...
          <div>
              ...
              {diasSemana.map((fecha, i) => {
                ...
                return (
                  <div key={fecha} className="py-1.5 text-center">
                    <p className="text-[10px] text-gray-500">{DIAS_SEMANA[i]}</p>
            ...
            <div className="max-h-[360px] overflow-y-auto">
              {HORAS.map((hora) => (
                <div key={hora} className="grid" style={{ gridTemplateColumns: "48px repeat(7, 1fr)", borderBottom: "0.5px solid #f3f4f6" }}>
                  ...
                  {diasSemana.map((fecha) => {
                    ...
                    return (
                      ...
                      >
                        {t.estado === "reservado" && <span className="text-[10px] font-medium text-white">Reservado</span>}
                        {t.estado === "bloqueado" && <span className="text-[10px] text-[#A32D2D]">—</span>}
                      </div>
                    );
                  })}
                </div>
