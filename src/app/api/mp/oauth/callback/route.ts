...
export async function GET(req: NextRequest) {
  ...
  if (tokenData.live_mode !== expectedLiveMode) {
    ...
    const whitelist = (process.env.MP_TEST_SELLERS_WHITELIST ?? "")
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean);
    const isWhitelisted = whitelist.includes(mpUserId);

    if (isWhitelisted) {
      logInfo("[OAUTH]", "Test seller OAuth aceptado en producción (whitelist)", {
        medicoId,
        mp_user_id: mpUserId,
