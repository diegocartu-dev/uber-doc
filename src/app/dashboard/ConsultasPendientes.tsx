...
export default function ConsultasPendientes({ medicoId, activa }: { medicoId: string; activa?: boolean }) {
  ...
  if (consultas.length === 0) return (
    <>
      {showRecordatorio && (
        <div className="mb-3 rounded-xl bg-amber-50 px-4 py-3 text-sm text-[#D85A30]" style={{ border: "0.5px solid #e5e7eb" }}>
          <p>Recordá desactivar tu disponibilidad si no podés atender en este momento.</p>
        </div>
      )}
      <div
              ...
              <div key={c.id} className="flex flex-col gap-2 rounded-lg p-4 transition hover:bg-gray-50 sm:flex-row sm:items-center sm:gap-4">
                ...
                <div className="flex gap-2 sm:flex-col">
                  ...
                  <TouchButton
                    ...
                    className="flex-1 shrink-0 rounded-lg border border-[#E24B4A] bg-transparent px-3.5 py-2 text-xs font-medium text-[#E24B4A] hover:bg-red-50 disabled:opacity-50 sm:w-auto min-h-[44px]"
                  >
                    Rechazar
                  </TouchButton>
                </div>
