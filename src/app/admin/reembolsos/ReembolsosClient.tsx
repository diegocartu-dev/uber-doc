...
const ESTADO_BADGE: Record<string, { label: string; bg: string; color: string }> = {
  pendiente: { label: "Médico sin saldo", bg: "#FDECEC", color: "#B23B3A" },
  fee_pendiente: { label: "Falta fee Docto", bg: "#FFF3E0", color: "#9A5A1A" },
};
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-semibold text-gray-900">Reembolsos</h1>
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-gray-100 text-left text-xs text-gray-400">
            <Th>Paciente</Th><Th>Médico</Th><Th>Monto</Th><Th>Estado</Th>
            <Th>Intentos</Th><Th>Próximo reintento</Th><Th>Desde</Th>
          </tr>
        </thead>
        <tbody>
          {rows.map((r) => {
            const badge = ESTADO_BADGE[r.estado] ?? { label: r.estado, bg: "#F3F4F6", color: "#6B7280" };
            return (
              <tr key={r.id} className="border-b border-gray-50 last:border-0">
                ...
                <Td>
                  <span className="rounded-full px-2 py-0.5 text-[11px] font-medium" style={{ background: badge.bg, color: badge.color }}>
                    {badge.label}
                  </span>
                </Td>
                <Td>{r.intentos}</Td>
        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r.medico_id} className="border-b border-gray-50 last:border-0">
              ...
              <Td className="text-[#1D9E75]">{money(r.total_recuperado)}</Td>
