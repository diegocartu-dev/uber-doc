...
>
> **Actualización 2026-06-23 — Mecánica de refund: UN refund total (SUPERSEDE el "dos refunds" de la sección 2.1).** El reembolso pasa a ejecutarse con **UN solo refund TOTAL** con el token del médico (collector), NO con dos refunds parciales. **Mercado Pago revierte automáticamente la comisión del marketplace** (`marketplace_fee`) desde la cuenta de Docto/GREBA al hacer la devolución total — confirmado por la **doc oficial de MP (split de pagos)** y por el **asistente oficial de MP** ("en una devolución total no tenés que hacer una devolución separada por la marketplace_fee; MP revierte la comisión del marketplace automáticamente; la reversión impacta en la cuenta del marketplace"). La premisa anterior ("MP no revierte el fee solo") era un **artefacto del sandbox** (collector y marketplace eran la MISMA cuenta de test → no había split que revertir). En producción, el segundo refund con el token de Docto fallaba con "Payment not found" (el token de GREBA no puede reembolsar un pago que vive en la cuenta del médico). **Implementado en PR #209** (`refundTotal` en `mp-refund.ts`; cron de reintentos reescrito; idempotency key única `refund:{tipo}:{id}`). El edge de saldo insuficiente (sección 2.2) y la cobertura CVU se mantienen igual. **Validación final pendiente:** confirmar contra el primer reembolso real (monitoreado) que el pago quede `refunded` y el fee se debite de GREBA. **Aprobado por Diego (2026-06-23).**
